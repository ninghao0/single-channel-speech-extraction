...
from .cnns import Conv1D, ConvTrans1D, TCNBlock, TCNBlock_Spk, ResBlock, TCNBlock_next, TCNBlock_Spk_next
from .spk_encoder import Spk_Encoder


class TDNext(nn.Module):
    def __init__(self,
                 L1=20,
                 L2=80,
                 L3=160,
        ...
        self.conv_block_1 = TCNBlock_Spk_next(spk_embed_dim=spk_embed_dim, in_channels=O, conv_channels=P, kernel_size=Q,
                                         causal=causal, dilation=1)
        self.conv_block_1_other = self._build_stacks_next(num_blocks=B, in_channels=O, conv_channels=P, kernel_size=Q,
                                                     causal=causal)
        self.conv_block_2 = TCNBlock_Spk(spk_embed_dim=spk_embed_dim, in_channels=256, conv_channels=512, kernel_size=3,
                                         causal=causal, dilation=1)
        self.conv_block_2_other = self._build_stacks(num_blocks=B, in_channels=256, conv_channels=512, kernel_size=3,
                                                     causal=causal)
        self.conv_block_3 = TCNBlock_Spk(spk_embed_dim=spk_embed_dim, in_channels=256, conv_channels=512, kernel_size=3,
                                         causal=causal, dilation=1)
        self.conv_block_3_other = self._build_stacks(num_blocks=B, in_channels=256, conv_channels=512, kernel_size=3,
                                                     causal=causal)
        self.conv_block_4 = TCNBlock_Spk(spk_embed_dim=spk_embed_dim, in_channels=256, conv_channels=512, kernel_size=3,
                                         causal=causal, dilation=1)
        self.conv_block_4_other = self._build_stacks(num_blocks=B, in_channels=256, conv_channels=512, kernel_size=3,
                                                     causal=causal)
        self.conv_block_5 = TCNBlock_Spk_next(spk_embed_dim=spk_embed_dim, in_channels=O, conv_channels=P, kernel_size=Q,
                                         causal=causal, dilation=1)
        self.conv_block_5_other = self._build_stacks_next(num_blocks=B, in_channels=O, conv_channels=P, kernel_size=Q,
                                                     causal=causal)
        ...
        self.conv1_r = Conv1D(O, 256, 1)
        self.conv2_r = Conv1D(256, O, 1)
        self.spk_encoder = Spk_Encoder(3 * N, 128, 384, spk_embed_dim)
    ...
    def _build_stacks_next(self, num_blocks, **block_kwargs):
        """
        Stack B numbers of TCN block, the first TCN block takes the speaker embedding
        """
        blocks = [
            TCNBlock_next(**block_kwargs, dilation=(2 ** b))
            for b in range(1, num_blocks)
        ]
        return nn.Sequential(*blocks)
    ...
    def forward(self, x, aux, aux_len):
        ...
        T = w1.shape[-1]
        xlen1 = x.shape[-1]
        xlen2 = (T - 1) * (self.L1 // 2) + self.L2  # //向下取整
        ...
        y = self.conv2_r(y)
        y = self.conv_block_5(y, aux)
        y = self.conv_block_5_other(y)
        ...
        return self.decoder_1d_short(S1), self.decoder_1d_middle(S2)[:, :xlen1], self.decoder_1d_long(S3)[:,
                                                                                 :xlen1], self.pred_linear(aux)
