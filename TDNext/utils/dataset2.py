...
class Dataset(object):
    ...
    def __init__(self, mix_scp="", ref_scp=None, aux_scp=None, noise_scp=None, spk_list=None, sample_rate=8000):

        # 拿到各个scp文件的字典形式 key - value (key - 路径)
        ...
        self.noise = WaveReader(noise_scp, sample_rate=sample_rate)
    ...
    def __getitem__(self, index):
        ...
        spk_idx = self.spk_list.index(key.split('-')[0])
        return {
...
class ChunkSplitter(object):
    ...
    def _make_chunk(self, eg, s):
        ...
        chunk["aux"] = eg["aux"]
        chunk["noise"] = eg["noise"][s:s + self.chunk_size]
    ...
    def split(self, eg):
        ...
        if N < self.chunk_size:
            ...
            chunk["aux"] = eg["aux"]
            chunk["noise"] = np.pad(eg["noise"], (0, P), "constant")
            chunk["aux_len"] = eg["aux_len"]
